...
async function main() {
  ...
  assert.equal(initial.users.find(user => user.uid === 'target').credit.monthlyFreeMilliseconds, 1800000);
  assert.equal(initial.users.find(user => user.uid === 'target').credit.purchasedMilliseconds, 0);
  assert(initial.users.find(user => user.uid === 'invited').credit.otherMilliseconds > 0);
  ...
  assert.equal(listed.users.find(user => user.uid === 'target').credit.promotionalMilliseconds, 7200000);
  assert.equal(listed.users.find(user => user.uid === 'target').credit.totalMilliseconds, 9000000);
  assert.deepStrictEqual(listed.users.find(user => user.uid === 'other').credit, initial.users.find(user => user.uid === 'other').credit);
  assert.deepStrictEqual(listed.users.find(user => user.uid === 'invited').credit, initial.users.find(user => user.uid === 'invited').credit);
  ...
  assert.equal(listed.users.find(user => user.uid === 'target').credit.monthlyFreeMilliseconds, 0);
  assert.equal(listed.users.find(user => user.uid === 'target').credit.promotionalMilliseconds, 6900000);
  await admin.addPromotionalHours(add);
  assert.equal((await admin.listUsers()).users.find(user => user.uid === 'target').credit.promotionalMilliseconds, 6900000, '消費後の再送も再付与しない');
  await admin.addPromotionalHours({ ...add, operationID: randomUUID(), hours: '1' });
  assert.equal((await admin.listUsers()).users.find(user => user.uid === 'target').credit.promotionalMilliseconds, 10500000);
  ...
  assert.equal((await broken.listUsers()).users.find(user => user.uid === 'target').credit, null, '取得失敗をゼロにしない');
